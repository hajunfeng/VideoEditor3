...
public class RecordedActivity extends BaseActivity implements View.OnClickListener, View.OnTouchListener, SensorControler.CameraFocusListener, SlideGpuFilterGroup.OnFilterChangeListener {
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                if (type == MagicFilterType.NONE){
                    Toast.makeText(RecordedActivity.this,"No filters are currently set--"+type,Toast.LENGTH_SHORT).show();
                }else {
                    Toast.makeText(RecordedActivity.this,"The current filter is toggled to--"+type,Toast.LENGTH_SHORT).show();
                }
            }
        });
        ...
        switch (v.getId()){
            ...
            case R.id.btn_camera_beauty:
                ...
                if (mCameraView.getCameraId() == 0){
                    Toast.makeText(this, "Rear camera can not use whitening function", Toast.LENGTH_SHORT).show();
                    return;
                }
                new AlertDialog.Builder(RecordedActivity.this)
                        .setSingleChoiceItems(new String[]{"close", "1", "2", "3", "4", "5"}, mCameraView.getBeautyLevel(),
    ...
    Runnable recordRunnable = new Runnable() {
        ...
        public void run() {
            ...
            String savePath = Constants.getPath("video/", time + ".mp4");
            ...
            try {
                ...
                if (timeCount < 2000) {
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(RecordedActivity.this, "The recording time is too short", Toast.LENGTH_SHORT).show();
                        }
                    });
                } else {
                    ...
                    Uri uri = Uri.fromFile(new File(savePath+File.separator));
                    intent.setData(uri);
    ...
    private void recordComplete(final String path) {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                Toast.makeText(RecordedActivity.this, "Video save path：" + path, Toast.LENGTH_SHORT).show();
            }
        });
