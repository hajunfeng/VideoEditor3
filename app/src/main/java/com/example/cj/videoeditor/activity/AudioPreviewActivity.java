    ...
    public void onClick(View v) {
        switch (v.getId()){
            ...
            case R.id.iv_confirm:
                ...
                previewView.pause();
                showLoading("processing...",false);

                final String path = Constants.getPath("video/outputAudio/", "audio_"+System.currentTimeMillis()+".mp3");//视频音乐
                AudioCodec.getAudioFromVideo(mVideoPath, path, new AudioCodec.AudioDecodeListener() {
                    @Override
                    public void decodeOver() {
                        //分离成功保存音频文件
                        Toast.makeText(AudioPreviewActivity.this,"succeed Audio save path---  "+path,Toast.LENGTH_SHORT).show();
                        Intent intent = new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE);
                        Uri uri = Uri.fromFile(new File(path+File.separator));
                        intent.setData(uri);
                    ...
                    public void decodeFail() {
                        //分离失败返回消息
                        Toast.makeText(AudioPreviewActivity.this,"failed maybe same Exception ，please look at logcat ",Toast.LENGTH_SHORT).show();
                        endLoading();
                    }
                });
