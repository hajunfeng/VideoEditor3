...
public class VideoSelectActivity extends BaseActivity implements LoaderManager.LoaderCallbacks<Cursor>,VideoAdapter.OnVideoSelectListener {
    ...
    public void onSelect(final String path, String cover) {
            ...
            for (int i = 0; i < extractor.getTrackCount(); i++) {
                ...
                if (format.getString(MediaFormat.KEY_MIME).startsWith("video/")) {
                    ...
                    if(!MediaFormat.MIMETYPE_VIDEO_AVC.equals(videoMime) ){
                        Toast.makeText(this,"The video format is not supported",Toast.LENGTH_SHORT).show();
                        return;
                    }
                    continue;
                }
        ...
        mDialog.setMessage("Split audio or add filter");
        mDialog.setPositiveButton("Add filler", new DialogInterface.OnClickListener() {
            @Override
        ...
        mDialog.setNegativeButton("Split audio", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                if(!TextUtils.isEmpty(path)){
